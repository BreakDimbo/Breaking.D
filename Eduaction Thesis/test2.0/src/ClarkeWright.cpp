...
void ClarkeWright::CreateSavingsMatrix(class VRP *V, double lambda, bool use_neighbor_list)
{
    ...
    ///
    ///为什么会出现lambda
    ///因为CW法的一个缺陷就是它在开始会建立好的解但是越往后解就越差
    ///lambda is a route shape parameter.
    ///the larger the lambda, the more emphasis is put on the distance between
    ///the vertices to be connected. It has been proved that using lambda=0.4or1 yields
    ///good solutions,taking into accout the number of routes and total length of the solution
...
bool ClarkeWright::Construct(VRP *V, double lambda, bool use_neighbor_list)
{
    ...
    num_routes=n;
    status=new unsigned char[V->num_original_nodes+1];  //每个节点是否被使用的状态

    memset(status,VRPH_UNUSED,sizeof(unsigned char)*(V->num_original_nodes+1)); //将status中
    //每个节点的状态设置为UNSED
    ...
    for(m=0; m<k; m++)
    {
        ...
        // Default is to skip
        x=-1;
        // used means whether or nor being merged
        ...
        if(status[i]==VRPH_INTERIOR || status[j]==VRPH_INTERIOR  )
            ...
            //interior means the node has been in the middle of two nodes
            //so it can not be connected again
            x=-1;
