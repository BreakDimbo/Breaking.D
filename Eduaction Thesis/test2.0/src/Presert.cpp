...
bool Presert::evaluate(class VRP *V, int u, int i, VRPMove *M)
{
    ...
    ///but why we compare them with 0?
    ///dose 0 represent depot?
    ///保证t-u-v在同一条路径上？
...
///if u and i are in the same route then the demand_change is 0
///and the i_length change is hu+ui+it+tv-(hi+it+tu+uv) = hu+ui-hi+tv-tu-uv
///=i_gain-u_loss, draw a picture and you'll see
    if(start_u==start_i)
    {
    ...
    // else it's feasible - record the move
     ...
     ///but what's the situation when start_u == end_u?
    ///此时是depot-u的情况，当u被移至i前后，depot-u路径消失。
    ...
    if(start_u==end_u)
     ...
     ///what's the difference between start_i==start_u and i_route==u_route
    if(u_route==i_route)
    {
    ...
    else
    {
        ...
        if(u!= V->dummy_index)        ///dummy_index?
        {
...
bool Presert::move(VRP *V, int u, int i)
{
    ...
    else
    {
        ...
        V->pred_array[VRPH_ABS(post_u)]=pre_u;    ///post_u不是>0吗？
    }
