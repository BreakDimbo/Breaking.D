...
bool Concatenate::evaluate(VRP *V, int i_route, int j_route, int rules, VRPMove *M)
{
    ...
    if(V->is_feasible(M, rules)==true)  //defined in VRP.cpp,check length and capacity
        return true;
    else
        return false;
...
bool Concatenate::move(VRP *V, int i_route, int j_route)
{
    ...
    V->next_array[j]=i;
    V->pred_array[i]=j;
//这里分情况主要是因为路径的order的存储方式
//即每条新路径的起点为负数
//将j路径上的所有点的路径编号变为i_route
    if(VRPH_ABS(route_after_j)==i)
    {
        current_node=start_j;
        while( (next_node= V->next_array[current_node]) > 0) //当指向新路径时，next_node <0
        {
        ...
        V->route_num[current_node]=i_route; //此时,current_node 为该路径的最后一个点。
...
//如果ij所在路径不相邻，则把i抽出来再与j相连。
    ...
    else
    {
        ...
        V->pred_array[VRPH_ABS(start_j)]=route_before_i;     //start_j 为何要加绝对值？
